...
class autoget:
    def getdriver():
        try:
            os.remove(str(pathlib.Path().absolute()) +
                      r'\Webdrivers\chromedriver_win32.zip')
            os.remove(str(pathlib.Path().absolute()) +
                      r'\chromedriver_win32.zip')
        ...
        wget.download(
            f'https://chromedriver.storage.googleapis.com/{versiondict[v]}/chromedriver_win32.zip')
        shutil.move(str(pathlib.Path().absolute())+r'\chromedriver_win32.zip',
                    str(pathlib.Path().absolute())+r'\Webdrivers')
